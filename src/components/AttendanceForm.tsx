...
const AttendanceForm = ({ onSuccess, onSubmit, initialData }: AttendanceFormProps) => {
	...
	const canGoNextFromDetails = () => {
		if (!formData.title) return false;
		if (attendanceType === 'class') {
			return !!(formData.program && formData.year);
		}
		return true;
	};
	...
	return (
		<div className="w-full max-w-none mx-auto px-6 py-4">
			...
			{/* Wizard steps */}
			<div className="flex items-center gap-2 mb-3 text-xs" aria-label="Form steps">
				{['Details', 'Schedule', 'Confirm'].map((label, idx) => {
					const n = idx + 1;
					const active = n === step;
					const done = n < step;
					return (
						<div key={label} className="flex items-center">
							<div className={`px-2 py-1 rounded ${active ? 'bg-primary text-white' : done ? 'bg-green-100 text-green-700' : 'bg-muted text-muted-foreground'}`}>{label}</div>
							{n < 3 && <ChevronRight className="w-3 h-3 mx-1 text-muted-foreground" aria-hidden="true" />}
						</div>
					);
				})}
			</div>
			...
			<div className="pb-8">
				<form onSubmit={handleSubmit} className="space-y-5" aria-live="polite">
					...
					{step === 3 && (
						<div className="rounded-md border border-border/50 p-4 text-sm space-y-2 bg-muted/30">
							<div className="font-medium">Review</div>
							<div>Title: {formData.title}</div>
							<div>Type: {attendanceType}</div>
							<div>Program/Year/Section: {formData.program || '—'} / {formData.year || '—'} / {formData.section || '—'}</div>
							<div>Date/Time: {formData.date || '—'} {formData.timeIn && `• ${formData.timeIn}`} {formData.timeOut && `- ${formData.timeOut}`}</div>
						</div>
					)}
					...
					<div className="md:col-span-2 pt-3 pb-4 flex gap-2 justify-center">
						...
						{step < 3 && (
							<Button type="button" disabled={step === 1 && !canGoNextFromDetails()} onClick={() => setStep((s) => Math.min(3, s - 1 + 2 - 1))} className="min-w-[160px]">Next</Button>
						)}
						{step === 3 && (
							<Button 
								type="submit" 
